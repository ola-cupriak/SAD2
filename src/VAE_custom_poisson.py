...
class EncoderNN(nn.Module):
    ...
    def forward(self, x) -> tuple():
        ...
        mu =  self.linear2(out)
        sigma = 1 + 0.2*F.softplus(self.linear3(out))
        return mu, sigma
...
class DecoderNN(nn.Module):
    ...
    def forward(self, z):
        out = self.linear1(z)
        out = F.relu(out)
        lamb = 1 + 0.2*F.softplus(self.linear2(out))
        return lamb
...
class VariationalAutoencoder(nn.Module):
    ...
    def forward(self, x) -> tuple():
        ...
        lamb, recon_loss = self.decoder(z, x)
