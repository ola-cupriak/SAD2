...
class DecoderNN(nn.Module):
    ...
    def forward(self, z):
        ...
        alpha = torch.exp(self.linear2(out)) 
        beta = torch.exp(self.linear3(out))
        return alpha, beta
...
class DecoderGaussian(nn.Module):
    ...
    def log_prob_xz(self, alpha, beta, x):
        """
    ...
    def forward(self, z,  x) -> tuple():
        ...
        recon_loss = self.log_prob_xz(alpha, beta, x)
        return alpha, beta, recon_loss
...
class VariationalAutoencoder(nn.Module):
    ...
    def forward(self, x) -> tuple():
        ...
        d_allpha, d_beta, recon_loss = self.decoder(z, x)
...
class scRNADataset(Dataset):
    ...
    def __init__(self, h5ad_file: str, sample: float, transform=None):
        ...
        min_new = 0.001
        max_new = 0.999
        self.data = ((max_new-min_new)/(self.data.max()-self.data.min())) * (self.data - self.data.max()) + max_new
...
def create_dataloader_dict(train_file: str, 
    ...
    train_dataloader = DataLoader(training_data, batch_size, shuffle, num_workers=4)
    test_dataloader = DataLoader(test_data, batch_size, shuffle, num_workers=4)
...
def plot_PCA_latent_space(z, output: str, ldim: int,
    ...
    plt.savefig(output+'_PCA.png', bbox_inches='tight')
